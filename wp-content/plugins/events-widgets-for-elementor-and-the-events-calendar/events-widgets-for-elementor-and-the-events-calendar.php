...
final class Events_Calendar_Addon {
	...
	public function ectbe_Install_ECT_Notice() {
		if ( current_user_can( 'activate_plugins' ) ) {
			...
			printf(
				...
				esc_html( __( 'In order to use our plugin, Please first install the latest version of', 'ectbe' ) ),
				sprintf(
	...
	function ectbe_fail_to_load() {
		if ( ! is_plugin_active( 'elementor/elementor.php' ) ) : ?>
			...
			<p><?php echo '<a href="https://wordpress.org/plugins/elementor/"  target="_blank" >' . esc_html__( 'Elementor Page Builder', 'ccew' ) . '</a>' . wp_kses_post( __( ' must be installed and activated for "<strong>Events Calendar Templates Builder For Elementor</strong>" to work', 'ectbe' ) ); ?></p>
			</div>
			<?php
		endif;
	}
